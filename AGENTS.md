...
- **`components/ui/bento-gallery.jsx`** (used by `CertificationsSection`) is a Tailwind + Motion (`motion/react`) component, not GSAP — it's the one deliberate exception to the GSAP-only rule, ported from a third-party (21st.dev) component. It only uses horizontal drag (`drag="x"`), deliberately with no wheel/touch listener of its own, so vertical wheel/touch scroll passes through untouched to `page.js`'s global section-snap. Don't add a wheel listener to it - an earlier version did, and it trapped the page inside the section.

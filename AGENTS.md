...
- **`CertificationsSection`** renders two vertical marquee columns (`components/ui/marquee-column.jsx`) around a fixed center panel. The scroll itself is a plain CSS `@keyframes` animation (not GSAP, not Motion) — deliberately, for compositor-thread performance on a continuous infinite loop. `components/ui/certificate-modal.jsx` is the one Motion (`motion/react`) usage left in this section, for the click-to-expand lightbox only.
